...
public class CWLCreator {
	...
	public CWLCreator(SolutionWorkflow solution, APEConfig apeConfig) {
		cwlRepresentation = new StringBuilder("class: WorkflowNo_" + solution.getIndex() + "\ncwlVersion: " + cwlVersion + "").append("\n");
	...
	private void defineCWLStep(ModuleNode moduleNode, APEConfig apeConfig, int i) {
		...
		for(TypeNode typeNode : moduleNode.getInputTypes()) {
			cwlRepresentation = cwlRepresentation.append(tabs(i+2) + (moduleNode.getNodeID() + (index++)) + "_" + typeNode.getNodeID() + ":");
	...
	private void getOperationRun(ModuleNode moduleNode, APEConfig apeConfig, int i) {
		...
		cwlRepresentation = cwlRepresentation.append(tabs(i+2) + moduleNode.getNodeLabel() + ": [" + moduleNode.getUsedModule().getPredicateID() + "]").append("\n");
		cwlRepresentation = cwlRepresentation.append(tabs(i) + "intent:").append(toYmlArray(moduleNode.getUsedModule().getSuperPredicates())).append("\n");
	...
	private void getNewCWLDataInstance(TypeNode typeNode, APEConfig apeConfig, int i) {
		...
		typeNode.getTypes().forEach(type -> { if(type.getRootNode().equals(formatRoot)) formats.add(type);});
