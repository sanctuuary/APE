...
public class Constraint_if_gen_then_type extends ConstraintTemplate {
	...
	public String getConstraint(List<ConstraintParameter> parameters, AllModules allModules, AllTypes allTypes, ModuleAutomaton moduleAutomaton,
		...
		GeneralEncodingUtils.getConjunctConstraints(ifType, parameterDimensions1, mappings, typeAutomaton, WorkflowElement.MEMORY_TYPE);
		...
		GeneralEncodingUtils.getConjunctConstraints(thenType, parameterDimensions2, mappings, typeAutomaton, WorkflowElement.MEMORY_TYPE);
		
		if (ifType == null || thenType == null) {
			System.err.println("Constraint argument does not exist in the type taxonomy.");
			return null;
		}
		constraint = SLTL_formula.ite_type(ifType, thenType, WorkflowElement.MEMORY_TYPE, moduleAutomaton, typeAutomaton.getMemoryTypesBlocks(), mappings);
