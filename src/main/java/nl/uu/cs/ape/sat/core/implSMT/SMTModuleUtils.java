...
public final class SMTModuleUtils {
	...
	public static String encodeMemoryStructure(SATSynthesisEngine synthesisInstance) {
		...
		constraints = constraints.append(enforceDataReferenceRules(synthesisInstance.getDomainSetup(),
				synthesisInstance.getTypeAutomaton(), synthesisInstance.getMappings()));
		return constraints.toString();
	}
	
	public static String encodeDataInstanceDependencyCons(TypeAutomaton typeAutomaton, AtomMappings mappings) {
		StringBuilder constraints = new StringBuilder();
		
		
		constraints = constraints.append(allowDataDependencyCons(typeAutomaton, mappings));
		constraints = constraints.append(enforceDataDependencyOverModules(typeAutomaton, mappings));
		constraints = constraints.append(enforceDataDependencyOverDataReferencing(typeAutomaton, mappings));
				...
				for (State moduleState : synthesisInstance.getModuleAutomaton().getAllStates()) {
					...
					for (State currInputState : currInputStates) {
						...
						if (currInputStateNo < moduleInputs.size()) {
							...
							constraints = constraints.append("-")
									.append(mappings.add(module, moduleState, WorkflowElement.MODULE)).append(" ");
							constraints = constraints.append(mappings.add(synthesisInstance.getEmptyType(),
	 ...
	 * @return String representation of constraints.
	 */
	private static String allowDataReferencingCons(TypeAutomaton typeAutomaton, AtomMappings mappings) {
	 ...
	 * @return String representation of constraints.
	 */
	private static String allowDataDependencyCons(TypeAutomaton typeAutomaton, AtomMappings mappings) {
	 ...
	 * @return String representation of constraints.
	 */
	private static String enforceDataDependencyOverDataReferencing(TypeAutomaton typeAutomaton, AtomMappings mappings) {
	...
	private static String enforceDataDependencyOverModules(TypeAutomaton typeAutomaton, AtomMappings mappings) {
		StringBuilder constraints = new StringBuilder();
	...
	private static String moduleEnforceTaxonomyStructureForState(AllModules allModules, TaxonomyPredicate currModule,
		...
		if (!(currModule.getSubPredicates() == null || currModule.getSubPredicates().isEmpty())) {
			...
			for (String subModule_State : subModulesStates) {
				currConstraint = currConstraint.append("-").append(subModule_State).append(" ").append(superModuleState)
						.append(" 0\n");
			}
	 ...
	 * @return
	 */
	public static String encodeWorkflowStructure(SMTSynthesisEngine smtSynthesisEngine) {

		StringBuilder constraints = new StringBuilder();

		// define that each tool state is defined over an operation
		for(State toolState : smtSynthesisEngine.getModuleAutomaton().getAllStates()) {
			constraints = constraints.append(SMTUtils.declareUnaryFun(toolState.getPredicateID(), "operation"));
		}
		
		// define that each type state is defined over each data dimension
		for(String dimension : smtSynthesisEngine.getDomainSetup().getAllTypes().getDataTaxonomyDimensions())
		for(State dataState : smtSynthesisEngine.getTypeAutomaton().getAllStates()) {
			constraints = constraints.append(SMTUtils.declareUnaryFun(dataState.getPredicateID(), "operation"));
		}
