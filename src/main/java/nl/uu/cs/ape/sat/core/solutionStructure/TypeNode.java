...
/**
 * The {@code MemTypeNode} class is used to represent a data instance in the memory, in the actual solution workflow.  
 * Each {@code MemTypeNode} represents an output of a tool in the workflow execution.
 * 
...
public class TypeNode extends SolutionWorkflowNode {
	...
	/** Type that represents the data instance. */
	private Set<Type> usedTypes;
	/** Abstract Types that describe the data instance. */
	private Set<Type> abstractTypes;
	/** Module/step in the workflow that generates the data instance as input. If {@code null} data instance corresponds to the initial workflow input. */
	private ModuleNode createdByModule;
	/** Modules/steps in the workflow that use this data instance as input. {@code NULL} represents workflow output. */
	...
	public TypeNode(State automatonState) throws Exception {
		...
		if(automatonState.getWorkflowStateType() != WorkflowElement.MEMORY_TYPE) {
			throw new Exception("Class MemTypeNode can only be instantiated using State that is of type WorkflowElement.MEMORY_TYPE, as a parameter.");
		}
	}
	 ...
	 * @param simpleType - primitive type that defines the instance.
	 */
	public void addUsedType(Type simpleType) {
		...
		if(simpleType.isSimpleType()) {
			...
			System.err.println("Abstract Type cannot be used to define an instance.");
		}
	}
	...
	/**
	 * Add a tool, that uses this data instance as input, to the list {@link TypeNode#usedByModules}.
	 * 
	...
	/**
	 * Get abstract data types that describe the instance. They are used in order to abstract from the simple/primitive data types.
	 ...
	 * @return {@link ModuleNode} that uses this data type.  If {@code null} data instance corresponds to the initial workflow input. 
	 */
	public ModuleNode getUsedByModule() {
		return createdByModule;
	}

	/**
	 * Get all module nodes that use the data instance in input. If {@code NULL} is in the set, 
	 * @return
	 */
