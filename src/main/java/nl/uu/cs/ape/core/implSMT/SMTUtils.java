...
public final class SMTUtils {
	 ...
	 * @throws IOException
	 */
	public static void appendToFile(File file, List<SMT2LibRow> smtRows, SMTPredicateMappings mapping) throws IOException {
		Writer fileWriter = new FileWriterWithEncoding(file, "ASCII", true);
		BufferedWriter writer = new BufferedWriter(fileWriter, 8192 * 4);
		for(SMT2LibRow row : smtRows) {
			writer.write(row.toString(mapping));
		}
		writer.close();
	}
	
	/**
	 * Remove all unsupported characters by SMT2Lib.
	 * @param text
	 * @return
	 */
