...
public class ExecutableCWLCreator extends CWLCreatorBase {
    /**
     * Keep track of the inputs that are available in the workflow (key: TypeNode
     * NodeID, value: input name).
     */
    private final HashMap<String, String> inputMap = new HashMap<>();
    /**
     * Keep track of the step names related to the ModuleNode NodeIDs (key: NodeID,
     * value: step name).
     */
    ...
    /**
     * A list of error messages generated by any missing CWL annotations related to
     * generating the CWL file.
     */
    private final ArrayList<String> missingAnnotationMessages;

    /**
     * Create executable cwl creator from an APE solution
     * 
    ...
    protected void generateCWLRepresentation() {
        ...
        // When some CWL annotations could not be found, list all missing annotations in
        // the output file
        if (!missingAnnotationMessages.isEmpty()) {
            ...
            for (String message : this.missingAnnotationMessages) {
                ...
                log.warn("Workflow {}: {}", solution.getIndex(), message);
            }
        }
    }
    ...
    private void generateInputs() {
        ...
        for (ModuleNode module : solution.getModuleNodes()) {
            ...
            // If a module does not have any input types, it is assumed it is a workflow
            // input
        ...
        // Keep track of how many times a certain input name appears. Used to prevent
        // duplicate names.
     ...
     * @return The CWL code with CWL annotations name for the input replaced by the
     *         new name.
     */
     ...
     * 
     * @param moduleNode     The {@link ModuleNode} that is represented in this
     *                       step.
     * @param implementation The CWL implementation of this step.
     * @return The CWL implementation as a string after giving all steps their
     *         definitive name.
     */
     ...
     * @return The CWL implementation as a string, where all inputs have the correct
     *         names.
     */
    private String connectStepInputs(ModuleNode moduleNode, String cwl) {
        ...
        while (matcher.find()) {
            ...
            /*
             * When a module did not have CWL inputs when it generated the inputs, it has
             * not been added to the input map.
             * The CWL file will already output all missing annotations, no need to show any
             * errors here.
    ...
    /**
     * Detect the output(s) of a workflow step and store them in the input map, so
     * they can be used as input in other steps.
     * 
    ...
    /**
     * Append the information that a module node is missing its CWL inputs
     * annotation.
     * 
     * @param moduleNode The ModuleNode that is missing its CWL inputs.
     */
    private void addMissingInput(ModuleNode moduleNode) {
        this.missingAnnotationMessages
                .add(String.format("Tool \"%s\" is missing its CWL inputs.", moduleNode.getNodeLabel()));
    }

    /**
     * Append the information that a module node is missing its CWL implementation
     * annotation.
     * 
    ...
    private void addMissingImplementation(ModuleNode moduleNode) {
        this.missingAnnotationMessages
                .add(String.format("Tool \"%s\" is missing its CWL implementation.", moduleNode.getNodeLabel()));
    }
}
