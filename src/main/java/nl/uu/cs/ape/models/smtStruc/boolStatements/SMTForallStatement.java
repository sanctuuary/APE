 ...
 *
 */
public class SMTForallStatement implements SMTFact {
	...
	public SMTForallStatement(SMTBoundedVar boundedVar, SMTDataType dataType, SMTFact content) {
	...
	public SMTForallStatement(List<SMTBoundedVar> boundedVars, List<SMTDataType> dataTypes, SMTFact content) {
		this.boundedVars = boundedVars;
		this.dataTypes = dataTypes;
		this.content = content;
	}
	
	public SMTForallStatement(List<SMTBoundedVar> boundedVars, List<SMTDataType> dataTypes, SMTFact content, boolean explicit) {
		this.boundedVars = boundedVars;
		this.dataTypes = dataTypes;
		this.content = content;
	...
	public String getSMT2Encoding(SMTSynthesisEngine synthesisEngine) {
		...
		if(additionalConstrints.size() > 0) {
			allRules = new SMTOrStatement(
							new SMTAndStatement(additionalConstrints), 
							content
						);
		}
	...
	private SMTBitVecComparisonStatement boundedVarIsInBounds(SMTBoundedVar boundedVar, SMTDataType dataType, SMTSynthesisEngine synthesisEngine) {
		return new SMTBitVecComparisonStatement(SMTBitVectorOp.GREATER_OR_EQUAL, boundedVar, new SMTBitVec(dataType, synthesisEngine.getAutomatonSize(dataType)));
	}
}
